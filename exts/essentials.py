...
class EssentialsConfigEntry(ConfigEntry):
    ...
    async def run(self, ctx):
        ...
        msgs = {"welcome": [tr["welcome1"], tr["welcome2"]], "goodbye": [tr["goodbye1"], tr["goodbye2"]]}
        try:
            for wg in msgs:
...
class Essentials(commands.Cog):
    ...
    async def status(self, ctx):
        ...
        tr = Translator(name, get_lang(ctx))
        stats = discord.Embed(name=tr["stats_name"], description=tr["stats_description"].format(ctx.guild.name, str(ctx.guild.created_at)[:10], ctx.guild.owner.name, ctx.guild.member_count - 1), color=7506394)
