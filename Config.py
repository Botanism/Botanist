...
class Config(commands.Cog):
    ...
    async def cfg_todo(self, ctx):
        local_logger.info("Starting todo configuration")
        try:
            await self.config_channels[ctx.guild.id].send("**Starting Todo configuration...**\n This extension lets you manage a todo list directly from discord. This extension requires more configuration than most. It has thus been subdivided for you. I'm going to ask which parts you want me to configure with you...")

            #asking for group configuration
            await self.config_channels[ctx.guild.id].send("Do you want to configure the todo groups ? [y/n]")
            response = await self.bot.wait_for("message", check=self.is_yn_answer)
            if response.content[0].lower() == "y": await self.cfg_todo_grps(ctx)

            #asking for channel configuration
            await self.config_channels[ctx.guild.id].send("Do you want to configure the todo channels ? [y/n]")
            reponse = await self.bot.wait_for("message", check=self.is_yn_answer)
            if response.content[0].lower() == "y": await self.cfg_todo_chan(ctx)
    ...
    async def cfg_todo_grps(self, ctx):
        try:
            ...
            while retry:
                ...
                await self.config_channels[ctx.guild.id].send(f"About to create group {grp_str}. Do you want to confirm this? [y/n]")
                response = await self.bot.wait_for("message", check=self.is_yn_answer)
                if response.content[0].lower() == "n":
                    continue
